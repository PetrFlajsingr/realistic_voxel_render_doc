...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:rt_1}
		...
		\ForEach{\text{primitive \textbf{in} scene}} {
			(intersected, distance) = intersect(ray, primitive)\;
			\If{intersected \textbf{and} distance < min\_distance}{
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:ray_marching}
		i = 0\;
		t = t\_min\;
		\While{t < t\_max \textbf{and} i < MAX\_ITERATIONS}{
			distance\_to\_scene = dist\_func(ray)\;
			\If{distance\_to\_scene < \textit{EPSILON}}{
				 \KwRet t\;
			}
			t += distance\_to\_scene\;
			i += 1\;
			ray.origin += distance\_to\_scene\;
		}
		\KwRet t\_max\;
...
\begin{table}[H]
	...
	\begin{tabular}{l|l}
		%\hline
		materiál              & dominantní distribuce \\ \Xhline{3\arrayrulewidth}
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:instanced_cube}
		...
		\ForEach{\text{voxel \textbf{in} voxel\_array}} {
...
\paragraph{Průsečík paprsku s~voxelem} lze počítat mnoha způsoby. Primitivní přístup k~řešení tohoto problému je počítat průsečík s~každou rovinou, která reprezentuje voxel a následně vybrat ten nejbližší (algoritmus \ref{alg:ray_box_primitive}). Tohle řešení je ale náročné a využívá podmínky, což může podle článku \cite{gpu_branch} značně zpomalit výpočet.
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:ray_box_primitive}
		...
		hit = false\;
		distance = \textit{inf}\;
		\ForEach{\text{coef \textbf{in} coefs}} {
			\If {coef >= 0} {
				hit = true\;
				hit\_point = ray.origin + ray.direction * coef;\;
				\If{is\_in\_box\_bounds(corner1, hit\_point)}{
					distance = coef\;
				}
			}
		}
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:bvh_traverse_naive}
		...
		stack = empty()\;
		exit = false\;
		\While{\textbf{not} exit} {
			\eIf{isInternalNode(current\_node)}{
			    \If{intersectNode(current\_node)}{
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:svo}
		...
		\While{\textbf{not} terminated}{
			(hit, t) = intersect\_cube(ray, voxel)\;
			\eIf{hit}{
				\If{is\_voxel\_small(voxel, pixel\_size) \textbf{or} is\_voxel\_leaf(voxel)}{
					\KwRet t\;
				}
				stack.push(voxel)\;
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{images/bvh_octree.png}
	\captionsetup{justification=centering}
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:single_probe_trace}
	    ...
	    \ForEach{segment \textbf{in} segments}{
	        coords\_on\_segment = create\_coords(segment)\;
	        \ForEach{coord \textbf{in} coords\_on\_segment}{
	            state = compare\_ray\_distance\_to\_radial(ray, coord, probe)\;
	            \If{state == HIT}{
	                \KwRet (HIT, coord)\;
	            }
	            \If{state == HIDDEN\_BEHIND\_SURFACE}{
	                \KwRet (UNKNOWN, coord) \;
	            }
	        }
	    }
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{images/probe_cube.png}
	\captionsetup{justification=centering}
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:light_field_trace}
	    ...
	    \While{result == UNKNOWN}{
	        ...
	        \If{\textbf{not} is\_valid\_probe(probe)}{
	            \textbf{break}\;
	        }
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:bounce_bake}
	    result = vector(1, 1, 1)\;
	    \For{i \textbf{in} range(0, MAX\_BOUNCES)}{
	        (is\_hit, position, normal, material\_type) = trace\_ray(ray)\;
	        \uIf{\textbf{not} is\_hit}{
	            \textbf{break}\;
	        }
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{images/gbuffer_render.png}
	\captionsetup{justification=centering}
...
Ukázka vytvoření tlačítka, které reaguje na kliknutí otevřením dialogu pro výběr \texttt{txt} souboru je ve výpisu \ref{lst:imgui}.

\begin{lstlisting}[language=C++, label={lst:imgui}, caption={Vytvoření tlačítka pro výběr souboru}]
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{images/program_flow.png}
	\captionsetup{justification=centering}
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:minimize_svo}
		...
		\Fn{\FIsFilled{$node$}}{
			\KwRet $node$.child\_count == 8 \textbf{and} all\_of($node$.children, IsFilled)\;
		}

		\Fn{\FMinTree{$node$}}{
			\eIf{IsNodeFilled($node$)}{
				node.children.clear()\;
				node.is\_terminal = true\;
			}{
				\ForEach{child \textbf{in} $node$.children}{
					MinimizeTree(child)\;
				}
			}
		}
...
\begin{center}
	\begin{czechalgorithm}[H] \label{alg:traverse_bvh_impl}
		...
		\Fn{\FTraverseBVH{$ray$}}{
		    ...
		    \While{intersected}{
		       \While{intersected \textbf{and} \textbf{not} intersected is leaf}{
		            ...
		            \If{intersection\_b hit \textbf{and} intersection\_b is closer than nearest geometry hit}{
		                push(stack, node.right\_child)\;
		            }
		            \If{neither node hit \textbf{and} \textbf{not} stack.Empty()}{
		                intersection\_a = pop(stack)\;
		            }
		        }
		        \If{intersected is leaf}{
        		    \If{intersection is closer than nearest geometry hit}{
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{images/load_diagram.png}
	\captionsetup{justification=centering}
...
\begin{table}[H]
	...
	\begin{tabular}{r|r}
		%\hline
		\multicolumn{1}{c|}{hloubka stromu} & \multicolumn{1}{c}{využití paměti {[}B{]}} \\ \Xhline{3\arrayrulewidth}
		1                                    & 20                                          \\ %\hline
		2                                    & 164                                         \\ %\hline
		3                                    & 1316                                        \\ %\hline
		4                                    & 10532                                       \\ %\hline
...
\begin{table}[H]
    ...
    \begin{tabular}{r|r}
    %\hline
    počet sond & využití paměti {[}MB{]} \\ \Xhline{3\arrayrulewidth}
    1          & 8.4                     \\ %\hline
    4          & 33.5                    \\ %\hline
    16         & 134.3                   \\ %\hline
    128        & 1074.8                  \\ %\hline
...
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=\textwidth]{images/12M_scene.png}
...
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=\textwidth]{images/render_svo_indirect_1.png}
...
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=\textwidth]{images/indirect_render_2.png}
...
Z pohledu efektivity algoritmů je načítání scén z formátů třetích stran poměrně pomalé, uživatel může čekat i několik vteřin u větších souborů, díky implementaci vlastního formátu je ale možné tomuto zpomalení předejít. Náročnost algoritmů na paměť je pro reprezentaci voxelových modelů je přijatelná, lze vykreslovat scény o velikosti desítek milionů voxelů s využitím paměti, se kterou by neměly problémy ani starší grafické karty -- například scéna obsahující \textasciitilde{}12 milionů voxelů využívá \textasciitilde{}92 MB paměti. Paměťová náročnost sond je výrazně vyšší, realistické využití s dalšími strukturami je pro maximální velikost mřížky sond přibližně 8x8x8. Paměťové omezení dané sondami implikuje, že navržený systém je vhodnější spíše pro scény v menší škále, ovšem s vysokou úrovní detailu.

Výsledné grafické výstupy tedy mohou obsahovat rozsáhlé scény, dochází ovšem ke vzniku artefaktů. Tyto artefakty jsou ovšem rušivé a s využitím použitých metod jim nejspíše nelze předejít. Alternativou k použitému přístupu by mohla být metoda uvedena v článku \textit{Dynamic Diffuse Global Illumination with Ray-Traced Irradiance Fields}, která navíc umožňuje využívat dynamických scén, na rozdíl od metody použité zde, která zvládá pouze scény statické. Mezi další rozšíření by mohla patřit implementace anti-aliasingu, specificky nějaká temporální metoda by byla zajímavá. Dalším vhodným rozšířením by byla implementace datové struktury, která by umožňovala optimalizované streamování dat scény z disku, aby bylo možné zobrazovat rozsáhlé scény bez nutnosti čekat na načtení modelů.
